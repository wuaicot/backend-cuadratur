import { MENUS, BEBESTIBLES, INGREDIENTES_MASTER } from '../../data/inventario';
...
export class AnalisisService {
    ...
    calcularConsumoTeorico(reporteZ) {
        const teoricoMap = new Map();
        for (const venta of reporteZ.ventas) {
            const producto = MENUS[venta.codigo] || BEBESTIBLES[venta.codigo];
            if (!producto)
                continue; // Si el producto no existe en nuestro inventario, lo ignoramos
            ...
            if ('ingredientes' in producto) {
                for (const ingredienteDef of producto.ingredientes) {
                    ...
                    teoricoMap.set(ingredienteDef.nombre, (teoricoMap.get(ingredienteDef.nombre) || 0) + cantidadConsumida);
                }
            }
            else {
                // Si es un bebestible, el "ingrediente" es el producto mismo
                const bebestible = producto; // Type assertion
                teoricoMap.set(bebestible.nombre, (teoricoMap.get(bebestible.nombre) || 0) + venta.cantidad);
            }
        }
        return teoricoMap;
    }
    calcularConsumoContado(planillaCaja, planillaCocina) {
        const contadoMap = new Map();
        ...
        for (const item of allInventarioItems) {
            ...
            contadoMap.set(canonicalName, (contadoMap.get(canonicalName) || 0) + consumoContado);
        }
        return contadoMap;
    }
    compararConsumos(teoricoMap, contadoMap) {
        ...
        for (const nombre of allIngredientNames) {
            ...
            let estado = 'OK';
            if (diferencia > 0.1)
                estado = 'SOBRANTE'; // Pequeño margen de error
            if (diferencia < -0.1)
                estado = 'FALTANTE';
            frontendItems.push({
                nombre,
