...
const EMPANADAS_PRODUCTS: EmpanadaDef[] = Object.values(EMPANADAS);

type Seccion = "BAR" | "COCINA" | "EMPANADAS" | "UNKNOWN";
type AnyProduct = MenuDef | BebestibleDef | EmpanadaDef;

const PRODUCT_LISTS: Record<Seccion, AnyProduct[]> = {
  BAR: BEBESTIBLES_PRODUCTS,
  COCINA: MENUS_PRODUCTS,
  EMPANADAS: EMPANADAS_PRODUCTS,
  UNKNOWN: [] as AnyProduct[],
};

const PRODUCT_NAMES: Record<Seccion, string[]> = {
...
const cleanLine = (line: string): string =>
  line.replace(/[^a-zA-Z0-9\s]/g, "").trim();
...
const getSectionForLine = (lineIndex: number, allLines: string[]): Seccion => {
  for (let i = lineIndex; i >= 0; i--) {
    ...
    if (lower.includes("empanadas")) return "EMPANADAS";
  }
  return "UNKNOWN";
};
...
export function parseReporteZ(texto: string): ParsedReporteZ {
  ...
  const ventas = lineas.flatMap((line, index) => {
    ...
    const productList: AnyProduct[] = PRODUCT_LISTS[section];
    const productNames: string[] = PRODUCT_NAMES[section];
    ...
    if (productInfo) {
      ...
      const numsInName: string[] = productInfo.nombre.match(/\d+/g) || [];

      let potential = allNums.filter((n) => !numsInName.includes(n));
      potential = potential.filter(
        (n) => !(n.length >= 4 && n !== productInfo.codigo)
      );
