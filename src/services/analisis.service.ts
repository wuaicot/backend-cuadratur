...
// Tipado
type PlanillaItem = { nombre: string; saldo: number; entrada?: number; total?: number };
type ParsedPlanilla = { tipo: "caja" | "cocina"; fecha: string | Date; items: PlanillaItem[] };
type VentaZ = { codigo: string; descripcion: string; cantidad: number };
type ParsedReporteZ = { fecha: string | Date; ventas: VentaZ[] };
...
export type ResultadoItem = {
  ...
  teorico: number;
  diferencia: number;
  estado: "OK" | "SOBRANTE" | "FALTANTE" | "NO_DATA";
  detalle?: string;
};

export class AnalisisService {
  analyze(
    planillaCaja: ParsedPlanilla | null,
    planillaCocina: ParsedPlanilla | null,
  ...
  private _mapVentasBebestibles(reporte: ParsedReporteZ) {
  ...
  private _mapVentasMenus(reporte: ParsedReporteZ) {
    const map: Record<string, number> = {};
  ...
  private _analizarCaja(planilla: ParsedPlanilla, ventasMap: Record<string, number>) {
    const results: ResultadoItem[] = [];

    const planillaMap = new Map<string, PlanillaItem>();
    for (const it of planilla.items) planillaMap.set(this._normalize(it.nombre), it);
  ...
  private _analizarCocina(planilla: ParsedPlanilla, ventasMenus: Record<string, number>) {
