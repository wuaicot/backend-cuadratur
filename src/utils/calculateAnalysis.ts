...
// Interfaz simple para salida (adáptala si tienes AnalysisItem en tu proyecto)
...
function normalizeStr(s: string | undefined) {
  if (!s) return "";
  return s
    .toLowerCase()
    .normalize("NFKD")
...
function extractVentasFromReporteZ(reporteData: any) {
  // si ya viene estructurado
  if (!reporteData) return [];
  if (Array.isArray(reporteData.ventas)) return reporteData.ventas;
  if (Array.isArray(reporteData.data?.ventas)) return reporteData.data.ventas;
  // fallback: buscar en ventas en cualquier lugar
  if (Array.isArray(reporteData)) return reporteData;
  return [];
}

// Extrae filas de planilla (intenta varios nombres posibles)
function extractRowsFromPlanilla(planillaData: any) {
  if (!planillaData) return [];
  if (Array.isArray(planillaData.items)) return planillaData.items;
  if (Array.isArray(planillaData.rows)) return planillaData.rows;
  if (Array.isArray(planillaData.parsed?.rows)) return planillaData.parsed.rows;
  if (Array.isArray(planillaData.parsed?.items)) return planillaData.parsed.items;
  return [];
}

// Dado un row intenta calcular contado con heurística:
// contado = SALDO_INICIAL + ENTRADA - DEVOLUCION - SALDO_FINAL
function computeContadoFromRow(row: any) {
...
function findRowByIngredient(rows: any[], ingredientName: string) {
  ...
  for (const r of rows) {
    ...
    for (const c of candidates) {
      if (!c) continue;
      if (normalizeStr(String(c)) === nTarget) return r;
    }
  }
  // 2) includes
  for (const r of rows) {
    ...
    for (const c of candidates) {
      ...
      if (nc.includes(nTarget) || nTarget.includes(nc)) return r;
    }
  }
  return null;
}
 ...
 */
export function calculateAnalysisFromBackendItems(backendItems: any[]): AnalysisItem[] {
  // localizar partes
  const reporte = backendItems.find((it: any) => it.tipo === "reporteZ")?.data || backendItems.find((it: any) => it.tipo?.toLowerCase?.().includes("reporte"))?.data;
  const cocina = backendItems.find((it: any) => it.tipo === "cocina")?.data || backendItems.find((it: any) => it.tipo?.toLowerCase?.().includes("cocina"))?.data;
  const caja = backendItems.find((it: any) => it.tipo === "caja")?.data || backendItems.find((it: any) => it.tipo?.toLowerCase?.().includes("caja"))?.data;
  ...
  const teoricoByIngredient: Record<string, number> = {};
  ...
  for (const ingName of Object.keys(teoricoByIngredient)) {
    ...
    let estado: AnalysisItem["estado"] = "OK";
    if (Math.abs(contado - teorico) < 0.0001) estado = "OK";
    else if (contado > teorico) estado = "SOBRANTE";
  ...
  if (result.length === 0 && rows.length > 0) {
    for (const r of rows) {
      ...
      const estado: AnalysisItem["estado"] = contado === 0 ? "OK" : (contado > 0 ? "SOBRANTE" : "FALTANTE");
